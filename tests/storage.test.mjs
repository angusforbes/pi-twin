...
test('durable state follows XDG_STATE_HOME and not the ephemeral runtime directory', t => {
  ...
  assert.equal(a, b); assert.equal(a, join(base, 'pi-twin')); assert.equal(statSync(a).mode & 0o777, 0o700);
});
test('state rejects symlink and non-private directory instead of changing access silently', t => {
  ...
  assert.throws(() => stateDir({ PI_TWIN_STATE_DIR: open }), /private/);
  const link = join(base, 'link'); symlinkSync(open, link);
  assert.throws(() => stateDir({ PI_TWIN_STATE_DIR: link }), /symlink/);
  assert.throws(() => stateDir({ PI_TWIN_STATE_DIR: '../relative' }), /absolute/);
});
