...
try {
  ...
  const env = { ...process.env, PATH: join(temp, 'bin') + ':' + process.env.PATH, PI_CODING_AGENT_DIR: join(temp, 'config'), PI_CODING_AGENT_SESSION_DIR: join(temp, 'sessions'), PI_LIVE_CLONE_STATE_DIR: join(temp, 'state'), XDG_RUNTIME_DIR: join(temp, 'runtime'), PI_OFFLINE: '1', PI_TELEMETRY: '0', HERDR_PANE_ID: 'wTest:p1', HERDR_SOCKET_PATH: join(temp, 'fake-herdr.sock'), LIVE_CLONE_TEST_URL: `http://127.0.0.1:${server.address().port}/v1` };
